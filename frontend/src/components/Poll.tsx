...
import { useReadForumGetPollFromPost, useWriteForumUpVotePollOption } from "../contracts/generated";
...
const Poll = ({ postId }: { postId: bigint }) => {
  ...
  const { isLoading: isVotingOption1, isSuccess: hasVotedOption1 } = useWaitForTransactionReceipt({
    hash: option1TxHash,
  });

  const { isLoading: isVotingOption2, isSuccess: hasVotedOption2 } = useWaitForTransactionReceipt({
    hash: option2TxHash,
  });
          ...
          <div className={styles.form}>
            ...
            <button
              ...
              {isVotingOption1 ? (
                ...
                <>
                  <FontAwesomeIcon icon={faArrowUpLong} /> {pollDetails?.option1}
                </>
              )}
              {": "}
            ...
            <button
              ...
              {isVotingOption2 ? (
                ...
                <>
                  <FontAwesomeIcon icon={faArrowUpLong} /> {pollDetails?.option2}
                </>
              )}
              {": "}
